...
root.config(bg="#040D12")

# se crea un grid para la pantalla
pantalla = tk.Frame(root)
pantalla.grid(column=0, row=0)
pantalla.config(bg="#040D12")

# TERMINAL
terminal = tk.LabelFrame(pantalla, text="terminal", border=2, fg="#525CEB", font=("Courier New", 12, "bold"))
terminal.grid(column=1, row=2, columnspan=3 ,padx=10, pady=10)
terminal.config(bg="#040D12")

# se crea el widget de las entradas del prompt
prompt_widget = tk.Entry(terminal, width= 100 ,font=("Courier New", 12, "bold" ), fg="#525CEB", highlightbackground="#525CEB", highlightcolor="#525CEB", highlightthickness=1, border=0)
prompt_widget.grid(column=1, row=2, padx=0, pady=5)
prompt_widget.config(bg="#040D12")

# se crea un widget para tener textos
text_widget = tk.Text(terminal, width=100, height=20, font=("Courier New", 12, "bold"), wrap="word",  fg="#525CEB",border=0)
text_widget.grid(column=1, row=1, padx=12, pady=0)
text_widget.config(bg="#040D12")
...
def handle_input(event):
    ...
    prompt_widget.delete(0, tk.END)  # Limpia la entrada despues de capturarlo
    ...
    if user_input == "instrucciones":
        ...
        try:
            # Ejecuta los comandos del sistema (los comandos de verdad)
            proceso = subprocess.run(user_input.split(), capture_output=True, text=True)
            output = proceso.stdout
            actualizar_display(output)

        except FileNotFoundError:
            output = f"No se reconoce este comando: {user_input}\n"
            actualizar_display(output)
